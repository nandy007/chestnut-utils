...
class DB {
     ...
     * @param   {Function}    success     [非必须，执行成功的回调]
     * @param   {Function}    error       [非必须，执行失败的回调]
     */
    query(sql, values, success, error){
        if(!this.db) return getNone();
        // 可以没有values，这时候回调函数顺位使用
        if(typeof values==='function'){
            error = success;
            success = values;
        }
        return this.db.query(sql, values, success, error).then(
            (rs) => {
                return rs;
            },
            (err) => {
